...
def user_normal_simmilarity(d):
    ...
    :param d: 字典形式为{user_id:{item_id:score,item_id:score}...}
    :return:
    '''
    w = dict()
    for u in d.keys():
        ...
        for v in d.keys():
            if u == v: continue
            dict_u= d[u] #{item_id:score,item_id:scorc}格式
            dict_v= d[v]
            # 用户商品列表的并集
            item_set = set(dict_u).union(dict_v)
            # 将{item_id:score,item_id:scorc}格式转化为向量形式
            vector_u=[]
            vector_v=[]
            for item_id in item_set:
                vector_u = vector_u.append(dict_u.get(item_id, 0))
                vector_v = vector_v.append(dict_v.get(item_id, 0))

            # 计算余弦相识度
            vstack = np.vstack([vector_u, vector_v])
            sim = 1 - pdist(vstack,'cosine')
            w[u][v] = sim
